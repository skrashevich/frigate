...
amd64:
	docker buildx build --push --platform linux/amd64 --target=frigate --tag $(IMAGE_REPO):$(VERSION)-$(COMMIT_HASH) --tag $(IMAGE_REPO):$(VERSION)-latest .
	docker buildx build --push --platform linux/amd64 --target=frigate-tensorrt --tag $(IMAGE_REPO):$(VERSION)-$(COMMIT_HASH)-tensorrt --tag $(IMAGE_REPO):$(VERSION)-tensorrt-latest .
