        ...
        for mount in mounts:
            ...
            if mount_info[1] == cgroup_path:
                ...
                if fs_type == "cgroup2fs" or fs_type == "cgroup2"  :
                    ...
                    logger.debug(f"Could not determine cgroups version: unhandled filesystem {fs_type}")
                break
            ...
            try:
                with open(f"/proc/{pid}/stat", "r") as f:
                    ...
                    if docker_memlimit > 0:
                        mem_res = rss * os.sysconf("SC_PAGE_SIZE") / 1024
                        mem_pct = str(round((float(mem_res) / float(docker_memlimit)) * 100, 1))
                    else:
                        mem_pct = str(round(rss / (os.sysconf("SC_PHYS_PAGES") * os.sysconf("SC_PAGE_SIZE")) * 100, 1))

                    usages[pid] = {
                        "cpu": str(round((utime + stime) / os.sysconf("SC_CLK_TCK"), 2)),
                        "mem": mem_pct,
                    }
            except:
                continue
